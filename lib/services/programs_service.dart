...
Future<List<ProgramModel>?> getEpg() async {
  try{
    ...
    List<ProgramModel> programs = objects.map((p) => ProgramModel(channelName: p["channel_name"], channelId: p["channel_uuid"], start: p["start"], stop: p["stop"], title: p["title"], subtitle: p["subtitle"], summary: p["summary"], description: p["description"], genreInt: p["genre"] == null ? [] : [int.parse(p["genre"])], channelNumber: p["channel_number"])).toList();
    return programs;
  }
  catch (e) {
    print(e);
